...
void mutt_str_pretty_size(struct Buffer *buf, size_t num)
{
  ...
  else if (!c_size_show_mb || (num < 1023949)) /* 10K - 999K */
  {
    ...
    int written = snprintf(buf->data, buf->dsize, c_size_units_on_left ? ("K%zu") : ("%zuK"), (num + 51) / one_kilobyte);
    buf->dptr += written;
  }
  else if (c_size_show_fractions && (num < 10433332)) /* 1.0M - 9.9M */
  {
    int written = snprintf(buf->data, buf->dsize, c_size_units_on_left ? "M%3.1f" : "%3.1fM", num / 1048576.0);
    buf->dptr += written;
  ...
  else /* 10M+ */
  {
    ...
    int written = snprintf(buf->data, buf->dsize, c_size_units_on_left ? ("M%zu") : ("%zuM"), (num + 52428) / 1048576);
    buf->dptr += written;
  }
