...
void mutt_str_pretty_size(struct Buffer *buf, size_t num)
{
  ...
  else if (!c_size_show_mb || (num < 1023949)) /* 10K - 999K */
  {
    ...
    snprintf(buf->data, buf->dsize, c_size_units_on_left ? ("K%zu") : ("%zuK"), (num + 51) / one_kilobyte);
  }
  ...
  else /* 10M+ */
  {
    ...
    snprintf(buf->data, buf->dsize, c_size_units_on_left ? ("M%zu") : ("%zuM"), (num + 52428) / 1048576);
  }
